...
class CONTRAfold(AbstractFold):
    def __init__(self, init_param=None):
        ...
        if init_param is None:
            ...
            self.score_bulge_0x1 = nn.Parameter(torch.zeros((4,), dtype=torch.float32))
            self.score_internal_1x1 = nn.Parameter(torch.zeros((4, 4), dtype=torch.float32))
            self.score_helix_stacking = nn.Parameter(torch.zeros((4, 4, 4, 4), dtype=torch.float32))
            self.score_helix_closing = nn.Parameter(torch.zeros((4, 4), dtype=torch.float32))
            ...
            self.score_dangle_left = nn.Parameter(torch.zeros((4, 4, 4), dtype=torch.float32))
            self.score_dangle_right = nn.Parameter(torch.zeros((4, 4, 4), dtype=torch.float32))
