...
class AbstractFold(nn.Module):
    ...
    def _create_modified_mask(
        self, seq: str, score_name: str, count_tensor: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        if count_tensor.dim() == 2:
            ...
            for idx in nonzero_indices:
                ...
                # Helix stacking score: involves (i,j) and (i+1,j-1) pairs
                    ...
                    if (
                        i in mod_positions
                        or j in mod_positions
                        or (i + 1) in mod_positions
                        or (j - 1) in mod_positions
                    ):
                        mask[i, j] = 1.0
    ...
    def calculate_differentiable_score(
        self,
        v: float,
        param: dict[str, Any],
        count: dict[str, Any],
        ...
        for n, p in param.items():
            if n.startswith("score_"):
                ...
                if self.modified_only and seq is not None:
                    cnt = self.apply_modified_only_mask(seq, "count_" + n[6:], cnt)

                s += torch.sum(p * cnt)
        s += -cast(torch.Tensor, s).item() + v
        return s

    def apply_modified_only_mask(
        self,
        seq: str | None,
        count_name: str,
        count_tensor: torch.Tensor,
    ) -> torch.Tensor:
        if not self.modified_only or seq is None or not count_name.startswith("count_"):
            return count_tensor
        score_name = "score_" + count_name[6:]
        mask = self._create_modified_mask(seq, score_name, count_tensor)
        return count_tensor * mask

    def make_param(
        self, seq: list[str], perturb: float = 0.0
    ...
    def forward(
        ...
        perturb: float = 0.0,
        loss_pos_paired: float | list[float] = 0.0,
        loss_neg_paired: float | list[float] = 0.0,
        loss_pos_unpaired: float | list[float] = 0.0,
        loss_neg_unpaired: float | list[float] = 0.0,
    ) -> (
        ...
        if param is None:
            ...
            if perturb > 0.0:
                param_temp = cast(
                    tuple[list[dict[str, Any]], list[dict[str, Any]]], param_temp
                )
        ...
        for i in range(len(seq)):
            ...
            with torch.no_grad():
                ...
                self.fold_wrapper.compute_viterbi(
                    seq[i],
                    param_on_cpu,
                    ...
                    loss_pos_paired=loss_pos_paired[i],
                    loss_neg_paired=loss_neg_paired[i],
                    loss_pos_unpaired=loss_pos_unpaired[i],
                    loss_neg_unpaired=loss_neg_unpaired[i],
                    pairing_rules=self.pairing_rules,
                )
                v, pred, pair = self.fold_wrapper.traceback_viterbi()

                if return_partfunc:
                    pf, bpp = self.fold_wrapper.compute_basepairing_probabilities(
                        seq[i],
                        param_on_cpu,
                        max_internal_length=max_internal_length
                        if max_internal_length is not None
        ...
        ss = (
            torch.stack(ss)
            if torch.is_grad_enabled()
            else torch.tensor(ss, device=device)
        )
        if return_param:
