...
from mxfold2.dataset import BPseqDataset, FastaDataset, ShapeDataset, RibonanzaDataset, JsonDataset, JsonShapeDataset
...
class Train(Common):
    ...
    def build_shape_loss_function(self, loss_func: str, model: AbstractFold, args: Namespace,
        ...
        if loss_func == 'shape_nll':
            ...
            return ShapeNLLLoss(model=model,
                            ...
                            sl_weight=args.score_loss_weight, shape_only=args.shape_only_training,
    ...
    def run(self, args: Namespace, conf: Optional[str] = None) -> None:
        ...
        if args.downsampling > 0.0:
            ...
            sampler = WeightedRandomSampler(weights, n_train_samples, replacement=False, generator=generator)

        train_loader = DataLoader(train_dataset, sampler=sampler, batch_size=1,
                                  shuffle=True if sampler is None else False, generator=generator) # works well only for batch_size=1!!
