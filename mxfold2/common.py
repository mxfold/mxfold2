...
class Common:
    ...
    def get_device(gpu: int) -> tuple[torch.device, str]:
        ...
        if gpu >= 0:
            if not torch.cuda.is_available():
                raise RuntimeError(
                    f"CUDA GPU {gpu} requested but CUDA is not available"
                )
    ...
    def build_model(self, args: Namespace) -> tuple[AbstractFold, dict[str, Any]]:
        if args.model == "Turner":
            if args.fold == "Zuker":
                ...
                if args.param == "default" or args.param == "turner2004":
                    ...
                    return LinFoldV(
                        init_param=param_turner2004, beam_size=args.beam_size
                    ), {}
                else:
        ...
        config = {
            ...
            if args.num_hidden_units is not None
            else (32,),
            "num_paired_filters": args.num_paired_filters,
            "paired_filter_size": args.paired_filter_size,
            "dropout_rate": args.dropout_rate,
            "dropout_rate_1d_cnn": args.dropout_rate_1d_cnn
            if args.dropout_rate_1d_cnn is not None
            else args.dropout_rate,
            "dropout_rate_2d_cnn": args.dropout_rate_2d_cnn
            if args.dropout_rate_2d_cnn is not None
            else args.dropout_rate,
            "dropout_rate_lstm": args.dropout_rate_lstm
            if args.dropout_rate_lstm is not None
            else args.dropout_rate,
        ...
        if args.model == "Positional":
            if args.fold == "Zuker":
                ...
                model = ZukerFold(modified_only=modified_only, **config)

            elif args.fold == "LinFold" or args.fold == "LinearFold":
                from .fold.linfold import LinFold

                model = LinFold(
                    beam_size=args.beam_size, modified_only=modified_only, **config
                )
            ...
            if args.fold == "Zuker":
                ...
                model = MixedFold(
                    init_param=param_turner2004, modified_only=modified_only, **config
                )
            ...
            if args.fold == "Zuker":
                ...
                model = CONTRAMixedFold(
                    init_param=param_contrafold202,
                    modified_only=modified_only,
                    **config,
                )
        ...
        if model is None:
            raise (
                NotImplementedError(
                    f"not implemented: model={args.model}, fold={args.fold}"
                )
            )
    ...
    def build_shape_model(self, args: Namespace) -> nn.Module:
        if args.shape_model == "Wu":
            ...
            return ShapeCNN(
                num_filters=args.shape_num_filters,
                filter_size=args.shape_filter_size,
                pool_size=args.shape_pool_size
                if args.shape_pool_size is not None
                else (1,),
    ...
    def add_fold_args(cls, subparser):
        ...
        gparser.add_argument(
            "--fold",
            choices=("Zuker", "LinearFold", "LinFold"),
            default="Zuker",
    ...
    def add_network_args(cls, subparser):
        ...
        gparser.add_argument(
            "--model",
            choices=("Turner", "CONTRAfold", "Positional", "Mix", "Mix1D", "CFMix"),
            default="Mix",
        ...
        gparser.add_argument(
            "--paired-opt", choices=("0_1_1", "fixed", "symmetric"), default="symmetric"
        )
        gparser.add_argument(
            "--mix-type", choices=("add", "average"), default="average"
        )
        gparser.add_argument(
            "--weight-turner",
            type=float,
        ...
        # Shape model settings (from modification branch)
        gparser.add_argument(
            "--shape-model",
            choices=("Wu", "Foo", "CNN"),
            default="Wu",
